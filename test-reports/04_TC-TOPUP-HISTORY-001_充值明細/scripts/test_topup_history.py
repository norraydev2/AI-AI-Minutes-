...
async def test():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 訪問充值明細頁面
            await page.goto(f"{BASE_URL}/top-up-history")
            await page.wait_for_timeout(3000)
            await page.screenshot(path=SCREENSHOT_DIR / "step1_history_page.png")
            steps.append({"step": 2, "desc": "充值明細頁面加載", "status": "通過"})
            results.append(True)

            # 驗證表格存在
            table = await page.query_selector("table, .ant-table, [class*=table]")
            steps.append({"step": 3, "desc": "充值記錄表格存在", "status": "通過" if table else "失敗"})
            results.append(True if table else False)

            # 驗證篩選條件
            filters = await page.query_selector_all("input[placeholder], select, .filter")
            await page.screenshot(path=SCREENSHOT_DIR / "step2_filters.png")
            steps.append({"step": 4, "desc": f"篩選條件：{len(filters)}個", "status": "通過" if filters else "失敗"})
            results.append(True if filters else False)
    ...
    print(f"TC-TOPUP-HISTORY-001: {passed}/{total} 通過")
