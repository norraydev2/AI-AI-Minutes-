...
async def test():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_timeout(3000)
            await page.screenshot(path=SCREENSHOT_DIR / "step1_page.png")
            ...
            if search_input:
                ...
                steps.append({"step": 3, "desc": "輸入搜索關鍵字", "status": "通過"})
                results.append(True)

                # 清空搜索
                await search_input.fill("")
                await page.wait_for_timeout(1000)
                steps.append({"step": 4, "desc": "清空搜索", "status": "通過"})
                results.append(True)
            else:
                steps.append({"step": 3, "desc": "搜索框不存在", "status": "跳過"})
                results.append(True)
    ...
    print(f"TC-MEETING-002: {passed}/{total} 通過")
