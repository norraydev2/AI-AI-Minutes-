...
async def test():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 測試 1: 輸入有效金額
            amount_input = await page.query_selector("input[type='number'], input[placeholder*='金額']")
            if amount_input:
                await amount_input.fill("100")
                await page.wait_for_timeout(1000)
                await page.screenshot(path=SCREENSHOT_DIR / "step2_valid_amount.png")
                steps.append({"step": 2, "desc": "輸入有效金額 100", "status": "通過"})
                results.append(True)
            else:
                steps.append({"step": 2, "desc": "金額輸入框不存在", "status": "失敗"})
            ...
            if amount_input:
                ...
                await page.wait_for_timeout(1000)
                await page.screenshot(path=SCREENSHOT_DIR / "step3_empty_amount.png")
                steps.append({"step": 3, "desc": "清空金額輸入", "status": "通過"})
                results.append(True)
            ...
            if amount_input:
                await amount_input.fill("-100")
                await page.wait_for_timeout(1000)
                await page.screenshot(path=SCREENSHOT_DIR / "step4_negative.png")
                steps.append({"step": 4, "desc": "輸入負數驗證", "status": "通過"})
                results.append(True)

            # 測試 4: 輸入非數字
            if amount_input:
                await amount_input.fill("abc")
                await page.wait_for_timeout(1000)
                await page.screenshot(path=SCREENSHOT_DIR / "step5_invalid.png")
                steps.append({"step": 5, "desc": "輸入非數字驗證", "status": "通過"})
                results.append(True)
    ...
    print(f"TC-TOPUP-002: {passed}/{total} 通過")
