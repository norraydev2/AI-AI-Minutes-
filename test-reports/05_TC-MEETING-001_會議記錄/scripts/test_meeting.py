...
async def test():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.wait_for_timeout(3000)
            await page.screenshot(path=SCREENSHOT_DIR / "step1_meeting_page.png")
            steps.append({"step": 2, "desc": "會議記錄頁面加載", "status": "通過"})
            results.append(True)

            # 驗證會議列表/卡片
            meetings = await page.query_selector_all("[class*=meeting], [class*=record], [class*=card], .ant-card")
            steps.append({"step": 3, "desc": f"會議記錄：{len(meetings)}個", "status": "通過" if meetings else "失敗"})
            results.append(True if meetings else False)

            # 驗證搜索/篩選
            search = await page.query_selector("input[placeholder*='搜索'], input[placeholder*='搜尋'], .search")
            await page.screenshot(path=SCREENSHOT_DIR / "step2_search.png")
            steps.append({"step": 4, "desc": "搜索功能存在", "status": "通過" if search else "失敗"})
            results.append(True if search else False)
    ...
    print(f"TC-MEETING-001: {passed}/{total} 通過")
