...
async def test():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 驗證側邊欄導航
            nav_items = await page.query_selector_all("aside a, nav a, .menu-item a, .sidebar a")
            await page.screenshot(path=SCREENSHOT_DIR / "step1_nav.png")
            steps.append({"step": 2, "desc": f"導航項目：{len(nav_items)}個", "status": "通過" if len(nav_items) >= 3 else "失敗"})
            results.append(True if len(nav_items) >= 3 else False)

            # 驗證頁面標題/品牌
            brand = await page.query_selector(".brand, .logo, [class*=brand], [class*=logo]")
            await page.screenshot(path=SCREENSHOT_DIR / "step2_brand.png")
            steps.append({"step": 3, "desc": "品牌 Logo 存在", "status": "通過" if brand else "失敗"})
            results.append(True if brand else False)
    ...
    print(f"TC-HOME-001: {passed}/{total} 通過")
